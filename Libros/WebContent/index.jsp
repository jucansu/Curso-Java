...
<body>
	...
	<%
		...
		Vector<avante.domain.Book> booklist = (Vector<avante.domain.Book>) session.getValue("ebookshop.list");
		if (booklist == null) {
			response.sendRedirect("/Libros/eshop");
		} else {
	%>
	...
	<%
		// Scriptlet 3: check whether the shopping cart is empty
			Vector shoplist = (Vector<avante.domain.Book>) session.getAttribute("ebookshop.cart");
